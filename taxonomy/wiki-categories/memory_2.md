# Memory

## Posts
- [Anatomy of a Program in Memory, 2009](http://duartes.org/gustavo/blog/post/anatomy-of-a-program-in-memory)
- [Motherboard Chipsets and the Memory Map, 2008](http://duartes.org/gustavo/blog/post/motherboard-chipsets-memory-map)
- [AMD Systems Programming Documentation](http://www.amd.com/us-en/Processors/DevelopWithAMD/0,,30_2252_739_7044,00.html)
- [LinuxMM](http://linux-mm.org/)

## Academic papers
- [Collected from: Memory safety](https://en.wikipedia.org/wiki/Memory_safety)
- ["Memory Safety Without Runtime Checks or Garbage C](http://llvm.org/pubs/2003-05-05-LCTES03-CodeSafety.pdf)
- ["How C Makes It Hard To Check Array Bounds"](http://www.drdobbs.com/cpp/how-c-makes-it-hard-to-check-array-bound/240168083)
- ["Practical memory safety for C"](https://www.cl.cam.ac.uk/techreports/UCAM-CL-TR-798.pdf)
- ["Computer Security Planning Study"](http://seclab.cs.ucdavis.edu/projects/history/papers/ande72.pdf)
- ["Memory Errors: The Past, the Present, and the Fut](https://www.isg.rhul.ac.uk/sullivan/pubs/tr/technicalreport-ir-cs-73.pdf)
- ["Defeating Solar Designer's Non-executable Stack P](http://insecure.org/sploits/non-executable.stack.problems.html)
- ["DieHard: Probabilistic Memory Safety for Unsafe L](http://www.cs.umass.edu/~emery/pubs/fp014-berger.pdf)
- ["DieHarder: Securing the Heap"](https://people.cs.umass.edu/~emery/pubs/ccs03-novark.pdf)
- ["Memory Debugging in Allinea DDT"](https://web.archive.org/web/20150203130856/https://www.allinea.com/user-guide/DDTMemoryDebugging.html)
- [the original](https://www.allinea.com/user-guide/DDTMemoryDebugging.html)
- ["Using Valgrind's Memcheck Tool to Find Memory Err](https://computing.llnl.gov/code/memcheck/)
- ["Memcheck: a memory error detector"](http://valgrind.org/docs/manual/mc-manual.html#mc-manual.mempools)
- ["Why custom allocators/pools are hard"](http://yosefk.com/blog/why-custom-allocatorspools-are-hard.html)
- ["Using the Garbage Collector as Leak Detector"](http://www.hboehm.info/gc/leak.html)
- ["Semantic Designs: CheckPointer compared to other ](http://www.semanticdesigns.com/Products/MemorySafety/MemorySafetyComparison.html)
- ["AddressSanitizerPerformanceNumbers"](https://github.com/google/sanitizers/wiki/AddressSanitizerPerformanceNumbers)
- ["A few billion lines of code later"](http://cacm.acm.org/magazines/2010/2/69354-a-few-billion-lines-of-code-later/fulltext)
- ["How to Avoid, Find (and Fix) Memory Errors in you](http://www.cprogramming.com/tutorial/memory_debugging_parallel_inspector.html)
- ["CWE-633: Weaknesses that Affect Memory"](https://cwe.mitre.org/data/definitions/633.html)
- ["CWE-762: Mismatched Memory Management Routines"](https://cwe.mitre.org/data/definitions/762.html)


## osdev
- [Bona fide OS developer](http://www.osdever.net/)
- [Memory Management](http://www.osdever.net/tutorials/index)
- [OSDev Wiki: Memory management](https://wiki.osdev.org/Memory_management)
- [Kernels](https://wiki.osdev.org/Kernels)
- [Page Frame Allocation](https://wiki.osdev.org/Page_Frame_Allocation)
- [Program Memory Allocation Types](https://wiki.osdev.org/Program_Memory_Allocation_Types)
- [Heap](https://wiki.osdev.org/Heap)
- [Garbage Collection](https://wiki.osdev.org/Garbage_Collection)
- [https://wiki.osdev.org/MMU](https://wiki.osdev.org/MMU)
- [Segmentation](https://wiki.osdev.org/Segmentation)
- [CPU Registers x86](https://wiki.osdev.org/CPU_Registers_x86#Segment_Registers)
- [Paging](https://wiki.osdev.org/Paging)
- [Detecting Memory (x86)](https://wiki.osdev.org/Detecting_Memory_(x86))
- [Garbage collection](https://wiki.osdev.org/Garbage_collection)
- [Memory Allocation](https://wiki.osdev.org/Memory_Allocation)
- [Writing a memory manager](https://wiki.osdev.org/Writing_a_memory_manager)
- [Brendan's Memory Management Guide](https://wiki.osdev.org/Brendan%27s_Memory_Management_Guide)

## Wikipedia
- [Mark-compact algorithm](https://en.wikipedia.org/wiki/Mark-compact_algorithm)
- [Memory leak](https://en.wikipedia.org/wiki/Memory_leak)
- [Memory management](https://en.wikipedia.org/wiki/Memory_management)
- [Memory management unit](https://en.wikipedia.org/wiki/Memory_management_unit)
- [Memory management as a function of OS](https://en.wikipedia.org/wiki/Memory_management_(operating_systems))
- [Manual memory management](https://en.wikipedia.org/wiki/Manual_memory_management)
- [Memory safety](https://en.wikipedia.org/wiki/Memory_safety)
- [Memory segmentation](https://en.wikipedia.org/wiki/Memory_segmentation)
- [C dynamic memory allocation](https://en.wikipedia.org/wiki/undefined)
- [Fragmentation (computing)](https://en.wikipedia.org/wiki/Fragmentation_(computing))
- [International Symposium on Memory Management](https://en.wikipedia.org/wiki/International_Symposium_on_Memory_Management)
- [Garbage (computer science)](https://en.wikipedia.org/wiki/Garbage_(computer_science))
- [Garbage collection (computer science)](https://en.wikipedia.org/wiki/Garbage_collection_(computer_science))
- [Garbage-first collector](https://en.wikipedia.org/wiki/Garbage-first_collector)
- [Boehm garbage collector](https://en.wikipedia.org/wiki/Boehm_garbage_collector)
- [Concurrent mark sweep collector](https://en.wikipedia.org/wiki/Concurrent_mark_sweep_collector)
- [Reference counting](https://en.wikipedia.org/wiki/Reference_counting)
- [Finalizer](https://en.wikipedia.org/wiki/Finalizer)
- [Static memory allocation](https://en.wikipedia.org/wiki/Static_variable)
- [Tracing garbage collection](https://en.wikipedia.org/wiki/Tracing_garbage_collection)
- [Unreachable memory](https://en.wikipedia.org/wiki/Unreachable_memory)

- [Virtual 8086 mode](https://en.wikipedia.org/wiki/Virtual_8086_mode)
- [Virtual memory compression](https://en.wikipedia.org/wiki/Virtual_memory_compression)
- [Virtual memory](https://en.wikipedia.org/wiki/Virtual_memory)
- [X86 memory segmentation](https://en.wikipedia.org/wiki/X86_memory_segmentation)
- [Page table](https://en.wikipedia.org/wiki/Page_table)
- [Paging](https://en.wikipedia.org/wiki/Paging)
- [Protected mode](https://en.wikipedia.org/wiki/Protected_mode)
- [Real mode](https://en.wikipedia.org/wiki/Real_mode)
- [Translation lookaside buffer](https://en.wikipedia.org/wiki/Translation_lookaside_buffer)
- [Demand paging](https://en.wikipedia.org/wiki/Demand_paging)

- [Buffer overflow](https://en.wikipedia.org/wiki/Buffer_overflow)
- [Buffer over-read](https://en.wikipedia.org/wiki/Buffer_over-read)
- [Dangling pointer](https://en.wikipedia.org/wiki/Dangling_pointer)
- [Stack overflow](https://en.wikipedia.org/wiki/Stack_overflow)


